...
const App = () => {
  ...
  const checkForColumnOfThree = () => {
    for (let i = 0; i <= 47; i++) {
      ...
      if (
        columnOfThree.every(
          (square) => currentColorArrangement[square] === decidedColour &&
          !isBlank
        )
      ) {
  ...
  const checkForColumnOfFour = () => {
    for (let i = 0; i <= 39; i++) {
      ...
      if (
        columnOfFour.every(
          (square) => currentColorArrangement[square] === decidedColour && !isBlank
        )
      ) {
  ...
  const dragStart = (e) => {
    //Pick out the id of the square that is being dragged
    //Save to a state
    setSquareBeingDragged(e.target);
  };

  const dragDrop = (e) => {
    //Pick out id of square that will be replaced
    //Save to a state
